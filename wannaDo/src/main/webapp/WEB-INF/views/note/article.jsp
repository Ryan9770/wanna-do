...
function deleteNote() {
	var query = "nums=${dto.noteNum}&${query}";
				...
				<div class="tab-pane fade show active mt-3" id="nav-content" role="tabpanel" aria-labelledby="nav-tab-content">
						...
						<tr>
							<td align="left" width="50%">
								<c:if test="${menuItem=='receive'}">
									...
									<button type="button" class="btn btn-light btnAccuseNote" onclick="accuseNote();">스팸신고</button>
								</c:if>
							</td>
							<td align="right">
					...
					<table class="table mb-0">
							...
							<td colspan="2" align="left">
								<c:choose>
									<c:when test="${menuItem=='receive'}">
										보낸사람 : ${dto.sendName}(${dto.sendId})
									</c:when>
									<c:when test="${menuItem=='send'}">
										받는사람 : ${dto.receiveName}(${dto.receiveId})
									</c:when>
								</c:choose>
							</td>
						</tr>
						...
						<tr>
							...
							<td align="right">
								읽은날짜 : ${empty dto.readDay ? "읽지 않음": dto.readDay}
							</td>
						</tr>
						...
						<tr>
							<td colspan="2" class="ellipsis">
								...
								<c:if test="${not empty preDto}">
									<span>&nbsp;<a class="text-reset" style="text-decoration-line:none;" href="${pageContext.request.contextPath}/note/${menuItem}/article?${query}&noteNum=${preDto.noteNum}"> ${preDto.subject} </a></span>
								</c:if>
							</td>
						</tr>
						<tr>
							<td colspan="2" class="ellipsis">
								...
								<c:if test="${not empty nextDto}">
									<span>&nbsp;<a class="text-reset" style="text-decoration-line:none;" href="${pageContext.request.contextPath}/note/${menuItem}/article?${query}&noteNum=${nextDto.noteNum}"> ${nextDto.subject} </a></span>
								</c:if>
							</td>
						</tr>
					</table>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="myDialogModalLabel">답변 보내기</h5>
			...
			<div class="modal-body">
				<form name="replyForm" method="post">
					<div class="row reply-form m-1">
						...
						<input type="hidden" name="receivers" value="${dto.sendId}">
						<input type="hidden" name="subject" value="RE:${dto.subject}">
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary btnSendOk">답변 보내기</button>
